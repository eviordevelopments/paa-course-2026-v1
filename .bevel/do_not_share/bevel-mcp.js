...
function hasFlag(
  flag,
  argv = globalThis.Deno ? globalThis.Deno.args : import_node_process2.default.argv,
) {
...
var Protocol = class {
          ...
          return (_a2 = this._transport) === null || _a2 === void 0
            ...
            : _a2.send({
                ...
                error: {
                  ...
                  message:
                    (_b2 = error.message) !== null && _b2 !== void 0 ? _b2 : "Internal error",
                },
              });
        },
      )
  ...
  request(request, resultSchema, options) {
    return new Promise((resolve, reject) => {
      ...
      (_c = options === null || options === void 0 ? void 0 : options.signal) === null ||
      _c === void 0
        ? void 0
      ...
      const timeout =
        (_d = options === null || options === void 0 ? void 0 : options.timeout) !== null &&
        _d !== void 0
          ? _d
...
function getExtensionUrl() {
  const portFilePath = import_path.default.join(__dirname, "port");
  const port = import_fs.default.readFileSync(portFilePath, "utf-8").trim();
  return `http://localhost:${port}`;
}
async function drawCallgraphRequest(startingFunction, filePath) {
  try {
    const response = await axios_default.get(`${getExtensionUrl()}/api/showGraph`, {
      headers: {
    ...
    console.log("showGraph successful:", response.data);
...
async function getDescriptionRequest(symbol, filePath, lineNumber) {
  try {
    const response = await axios_default.get(`${getExtensionUrl()}/api/getDescription`, {
      headers: {
        "Content-Type": "application/json",
      },
      params: {
    ...
    console.log("getDescription successful:", response.data);
    return response.data;
  } catch (error) {
...
var getTools = [
  /*{
      name: 'draw_callgraph',
      description: 'Display a callgraph starting at the specified function. This will return additional information about what other functions are called by this function, in order to help you understand what this function does.',
      inputSchema: {
        type: 'object',
        properties: {
          startingFunction: {
              type: 'string',
              description: 'Header of the function where the callgraph should start, this should contain its name and its parameters, including the parentheses, exactly as they appear in the function declaration in the code, as the function will be searched via string comparison.'
          },
          filePath: {
              type: 'string',
              description: 'The relative or absolute file path of the file, where the startingFunction is declared'
          },
        },
        required: ['startingFunction', 'filePath']
      }
  },*/
  {
    name: "get_documentation",
    description:
      "Get documentation relevant to the specified symbol. It can be a function, a class, or a variable. Use this when wanting to understand something, for example for testing or explanation purposes.",
    inputSchema: {
      type: "object",
      properties: {
        symbol: {
          type: "string",
          description:
            "Symbol name as it appears in the its declaration in the code. In the case of functions this should contain its name and its parameters, including the parentheses, exactly as they appear in the function declaration in the code, as the symbol will be searched via string comparison.",
        },
        filePath: {
          type: "string",
          description:
            "The relative or absolute file path of the file, where the symbol is declared. NOT where its used, but where its declared.",
        },
        lineNumber: {
          type: "integer",
          description:
            "The line number where the symbol is declared. NOT where its used, but where its declared.",
        },
      },
...
async function getDocumentation(args) {
  if (!args.symbol || !args.filePath) {
    throw new McpError(
      ...
      "Both startingFunction and filePath are required for getting the symbol documentation",
    );
  }
  const symbol = args.symbol;
  const filePath = args.filePath;
  const description = await getDescriptionRequest(symbol, filePath, args.lineNumber);
  return {
    content: [
      {
        type: "text",
        text: description ?? "No documentation found",
      },
    ],
  };
}
async function drawCallgraph(args) {
  if (!args.startingFunction || !args.filePath) {
    throw new McpError(
      ErrorCode.InvalidParams,
      "Both startingFunction and filePath are required for getting the symbol documentation",
    );
  }
  const startingFunction = args.startingFunction;
  const filePath = args.filePath;
  console.log("drawCallgraph", startingFunction, filePath);
  const callgraph = await drawCallgraphRequest(startingFunction, filePath);
  return {
    content: [
      {
        type: "text",
        text: JSON.stringify(callgraph),
      },
    ],
  };
}
...
var BevelServer = class {
  ...
  setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        if (Object.keys(functionCallbacks).find((key) => key === request.params.name)) {
          return await functionCallbacks[request.params.name](request.params.arguments);
        }
        ...
        if (axios_default.isAxiosError(error)) {
          throw new McpError(
            ErrorCode.InternalError,
            `Bevel API error: ${error.response?.data?.message || error.message}`,
          );
        }
        throw error;
      }
  ...
  async run() {
    ...
    console.log("Bevel Requirements MCP server running on stdio");
  }
};
var server = new BevelServer();
...
mime-types/index.js:
   ...
   *)
*/
//# sourceMappingURL=bevel-mcp.js.map
